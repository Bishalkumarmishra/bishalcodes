...
const App: React.FC<AppProps> = ({ initialSlug = [] }) => {
  ...
  useEffect(() => {
    if (typeof document !== 'undefined') {
      ...
      const isCyberDefender = currentPage === 'services' && selectedId === 'cyber-defender';
      
      if (isFileTransfer || isCalendarTool || isJpgToPdf || isIosProfile || isCyberDefender) {
  ...
  return (
    <NavigationContext.Provider value={{ currentPage, selectedId, navigate }}>
      <div className={`antialiased font-sans min-h-screen ${selectedId === 'ios-profile' ? 'bg-[#050507] text-white' : 'bg-slate-50 text-slate-800'}`}>
        {renderContent()}
