...
export const ExcelToPdfConverter: React.FC = () => {
  const { navigate } = useNavigation();
  const [excelFile, setExcelFile] = useState<{ id: string; file: File; name: string; size: number } | null>(null);
  ...
  const handleFiles = useCallback(async (files: FileList | File[] | null) => {
    ...
    if (extension !== 'xlsx' && extension !== 'xls') {
      setError('Only Excel documents (.xlsx, .xls) are supported.');
      return;
    }
  ...
  const processExcel = async () => {
    ...
    try {
      ...
      const base64Excel = await new Promise<string>((resolve, reject) => {
      ...
      setProgress(40);
      setGenerationStep('Converting sheet layouts and charts on server...');

      const response = await fetch('/api/excel-to-pdf-internal', {
        method: 'POST',
        headers: {
      ...
      const outName = excelFile.name.replace(/\.(xlsx|xls)$/i, '') + '.pdf';
  ...
  if (downloadUrl) {
    return (
      <div className="w-full font-sans">
        <ToolDownloadStep
          title="The Excel spreadsheet has been converted to PDF"
          downloadUrl={downloadUrl}
          downloadFileName={downloadName}
          onReset={handleReset}
        />
        <SeoGuideSection toolId="excel-to-pdf" />
      </div>
    );
  }
      ...
      {!excelFile ? (
        <div className="w-full px-4 md:px-8 xl:px-12 pb-12">
          <ToolHeroUpload
            ...
            description="Convert Excel spreadsheets (.xlsx, .xls) into high-fidelity PDF documents with exact layout formatting, table alignments, and charts."
            buttonText="Select Excel file"
            dropText="or drop Excel file here"
            accept=".xlsx,.xls"
            multiple={false}
        ...
        <div className="w-full px-4 md:px-8 xl:px-12 pb-12">
          ...
          <div className="max-w-4xl mx-auto bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-xl">
            <div className="flex flex-col md:flex-row items-center gap-8 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
              <div className="w-20 h-20 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0">
                <FileSpreadsheet size={40} />
              </div>
              <div className="flex-1 text-center md:text-left min-w-0">
                <h3 className="text-xl font-extrabold text-slate-900 dark:text-white truncate mb-1" title={excelFile.name}>
                  {excelFile.name}
                </h3>
                <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                  Size: {formatSize(excelFile.size)}
                </p>
              </div>
              <button
                onClick={handleReset}
        ...
        <div className="w-full max-w-4xl mx-auto bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/40 rounded-xl p-4 flex gap-3 text-xs leading-normal">
          ...
          <div>
            ...
            <span className="text-red-800/95 dark:text-red-400/80 font-medium">
              Your files are secure. Excel spreadsheet layout rendering and PDF packaging are processed entirely in a secure environment. Your uploads are instantly deleted from storage post-conversion.
            </span>
          </div>
        </div>
      </div>
