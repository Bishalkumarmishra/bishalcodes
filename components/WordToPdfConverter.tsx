...
export const WordToPdfConverter: React.FC = () => {
  const { navigate } = useNavigation();
  const [wordFile, setWordFile] = useState<{ id: string; file: File; name: string; size: number } | null>(null);
  ...
  const handleFiles = useCallback(async (files: FileList | File[] | null) => {
    ...
    if (extension !== 'docx' && extension !== 'doc') {
      setError('Only Word documents (.docx, .doc) are supported.');
      return;
    }
  ...
  const processWord = async () => {
    ...
    try {
      ...
      const base64Docx = await new Promise<string>((resolve, reject) => {
        ...
        reader.readAsDataURL(wordFile.file);
      });

      setProgress(40);
      setGenerationStep('Converting document layouts and typography on server...');

      const response = await fetch('/api/word-to-pdf-internal', {
        method: 'POST',
        headers: {
      ...
      setProgress(85);
      setGenerationStep('Downloading PDF document...');
      
      const pdfBlob = await response.blob();
      const url = URL.createObjectURL(pdfBlob);
      const outName = wordFile.name.replace(/\.(docx|doc)$/i, '') + '.pdf';
  ...
  if (downloadUrl) {
    return (
      <div className="w-full font-sans">
        <ToolDownloadStep
          title="The Word document has been converted to PDF"
          downloadUrl={downloadUrl}
          downloadFileName={downloadName}
          onReset={handleReset}
        />
        <SeoGuideSection toolId="word-to-pdf" />
      </div>
    );
  }
      ...
      {!wordFile ? (
        <div className="w-full px-4 md:px-8 xl:px-12 pb-12">
          <ToolHeroUpload
            ...
            description="Convert Word documents (.docx, .doc) into high-fidelity PDF documents with exact margins, layout preservation, and font embedding."
            buttonText="Select Word file"
            dropText="or drop Word file here"
            accept=".docx,.doc"
            multiple={false}
        ...
        <div className="w-full px-4 md:px-8 xl:px-12 pb-12">
          ...
          <div className="max-w-4xl mx-auto bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-xl">
            <div className="flex flex-col md:flex-row items-center gap-8 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
              <div className="w-20 h-20 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0">
                <FileText size={40} />
              </div>
              <div className="flex-1 text-center md:text-left min-w-0">
                <h3 className="text-xl font-extrabold text-slate-900 dark:text-white truncate mb-1" title={wordFile.name}>
                  {wordFile.name}
                </h3>
                <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                  Size: {formatSize(wordFile.size)}
                </p>
              </div>
              <button
                onClick={handleReset}
        ...
        <div className="w-full max-w-4xl mx-auto bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/40 rounded-xl p-4 flex gap-3 text-xs leading-normal">
          ...
          <div>
            ...
            <span className="text-red-800/95 dark:text-red-400/80 font-medium">
              Your files are secure. Word document layout rendering and PDF packaging are processed entirely in a secure environment. Your uploads are instantly deleted from storage post-conversion.
            </span>
          </div>
        </div>
      </div>
